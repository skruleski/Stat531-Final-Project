...
ca <- filter(states, region == "california")

ggplot(data = ca, aes(x = long, y = lat)) +
  geom_polygon(fill = "white", color = "grey25", linewidth = 0.1) +
  coord_cartesian() +
  geom_point(data = filter(full, y > 0), aes(x = x, y = y), color = "steelblue", alpha = 0.1) +
  labs(title = "Map of recorded frog data in California",
       x = "Latitude",
       y = "Longitude")
```
